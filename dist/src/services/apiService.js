...
/** Get a player's Course Handicap at every tee of a given course (match-start tee picker). Null
 * if the player has no handicap on file, or the course has no known GHIN linkage. */
const getPlayerCourseHandicaps = async (playerId, courseId) => {
    try {
        const response = await fetch(`${API_URL}/ryder/players/${playerId}/course-handicaps?courseId=${courseId}`);
        if (!response.ok)
            return null;
        return (await response.json());
    }
    catch (error) {
        console.error('Error fetching player course handicaps:', error);
        return null;
    }
};
...
const saveMatchPlayerTee = async (year, groupId, matchId, playerId, ghinTeeSetId, teeName, courseHandicap) => {
    try {
        const response = await fetch(`${API_URL}/ryder/match-tees`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ year, group: groupId, match: matchId, playerId, ghinTeeSetId, teeName, courseHandicap }),
        });
        return response.ok;
    }
    catch (error) {
        console.error('Error saving match tee:', error);
        return false;
    }
};
...
const getMatchHoleScores = async (year, groupId, matchId, hole) => {
    try {
        const response = await fetch(`${API_URL}/ryder/match-hole-scores?year=${year}&group=${groupId}&match=${matchId}&hole=${hole}`);
        if (!response.ok)
            return {};
        return (await response.json());
    }
    catch (error) {
        console.error('Error fetching match hole scores:', error);
        return {};
    }
};
...
const saveMatchHoleScores = async (year, groupId, matchId, hole, scores) => {
    try {
        const response = await fetch(`${API_URL}/ryder/match-hole-scores`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ year, group: groupId, match: matchId, hole, scores }),
        });
        return response.ok;
    }
    catch (error) {
        console.error('Error saving match hole scores:', error);
        return false;
    }
};
...
const verifyCaptainPassword = async (password) => {
    try {
        ...
        console.error('Error verifying captain password:', error);
        return false;
    }
};
